...
Note that NEE is multiplied with -1 to change the sign of the flux. A positive sign means carbon absorbed by the ecosystem.

```{r Load Muka Head Data}
Muka_Head <- data.frame(DateTime = df$date, NEE = -1 * df$FCO2, Ustar = df$USTAR,
                   Rg = df$RG, Tair = df$TA_ema, rH = df$RH)
```

# Check the Data for Continuity of DateTime

We cannot create the REddyProc object if the DateTime variable is not continuous. I use the `openair` package and the `timeAverage` function to fill in the missing timestamps.
...
Create the data object for the Gebesee data. The ID is `MY-MkH` and the parameters are:
...
However, for Muka Head, $u_*$ should not change appreciably because the water surface condition was unvarying. 
...
plot(head(Muka_Head2$DateTime, nRec), 
     ...
     type = "l", xlab = 'Time', ylab = 'GPP_DT_U50')
lines(head(Muka_Head2$DateTime, nRec), 
