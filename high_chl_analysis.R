...
abline(v = as.POSIXct("2020-03-20 08:00:00", format = "%Y-%m-%d %H:%M:%S"), lty = 2)
...
abline(v = as.POSIXct("2020-02-28 08:00:00", format = "%Y-%m-%d %H:%M:%S"), lty = 2)
...
plot(high_chl_day3$date,high_chl_day3$FCO2, pch=19, ylim = c(-0.5,0.5))
abline(h=0)

# Pick one day. I picked 2019-12-28.
high_chl_day4 <- selectByDate(df, start = "28/12/2019", end = "28/12/2019")
plot(high_chl_day4$date,high_chl_day4$FCO2, pch=19, ylim = c(-0.5,0.5))
abline(h=0)
